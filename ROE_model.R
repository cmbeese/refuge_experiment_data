#### Model Function ##################################################################################
# Author: Alice Rogers
# Last Update: 11/11/21, Chelsey Beese
# Purpose: Modified model refuge function to include 10 bins
#######################################################################################################
...
run_model <- function(params, initial.run = FALSE, fish.herbs = TRUE){
  ...
  with(params, {
    ...
    # and fish prey body size: diet preference (in the predator spectrum only)
    phi.f = function(q){
      q = q
      q0 = q0
      sd.q = sd.q
    ...
    #### Availability function (REFUGE) ############################################################
  
    vulnerability = function(x){
      ifelse(x < settle, 1,
             ifelse(x < end1, bin1,
             ifelse(x >= start2 & x < end2, bin2,
             ifelse(x >= start3 & x < end3, bin3,
             ifelse(x >= start4 & x < end4, bin4,
             ifelse(x >= start5 & x < end5, bin5,
             ifelse(x >= start6 & x < end6, bin6,
             ifelse(x >= start7 & x < end7, bin7,
             ifelse(x >= start8 & x < end8, bin8,
             ifelse(x >= start9 & x < end9, bin9,
             ifelse(x >= start10 & x < end10, bin10,elseAv)))))))))))
    }

    # Alice's old step function
    
    # vulnerability = function(x){
    #   ifelse(x<settle, 1, (1-min.A)/(1+exp(-emsp*(x-emerge)))+min.A)
    # }
    
    
    # Current iteration, based on data
    ...
    ## predators
    death.u<-function(u,A,expax,mphi,a.u,pref.pel){return((a.u*pref.pel*A*expax)*(u*dx)%*%(mphi))}    
    ...
    out.w<-function(A.v,A.h,v,w,h,alpha.h,pref.det){
      return(sum((A.v*(10^(x*0.75))*w*v)*dx)+sum((A.h*pref.det*(10^(x*alpha.h))*w*h)*dx))
      } 
    # biomass density removed by detritivores per year    
    ...
    ## algae output (g.m-3.yr-1)
    out.a <- function(A.h, alpha.h, h, a, pref.alg){
      return(sum(pref.alg*A.h*(10^(x*alpha.h)*a*h)*dx))
      } 
     # biomass density removed by herbivores ?per day if using daily data or per hyear if using yearly data? 
    ...
    # matrix for keeping track of reproduction from ingested food
    ...
    a.u <- vulnerability(10^x)
    a.h <- vulnerability(10^x)
    a.v <- 1  
    ...
    for (i in 1:(N-1)) {
      ...
      f.pel <- as.vector((a.u*pref.pel*A.u*expax)*(U[,i]*dx)%*%(gphi))
      
      # Feeding on herbivores
      f.herb <- as.vector((a.h*pref.herb*A.u*expax)*(H[,i]*dx)%*%(gphi))      
      
      #F eeding on invertebrates
      ...
      # Predation mortality
      PM.u[,i] <- as.vector(death.u(U[,i],A.u,expax,mphi,a.u,pref.pel))
      Z.u[,i]  <- PM.u[,i]+ OM.u + SM.u + Fvec_pred
      
      # Herbivore death integral
      PM.h[,i] <- as.vector(death.h(U[,i],A.u,expax,mphi,a.h,pref.herb))
      ...
      #### Predators Eat and Poo ####
      
      # biomass density eaten by predators (g.m-3.yr-1)
      eatenbypred <- 10^x*(a.u*pref.pel*A.u*expax*(U[,i]%*%gphi)*U[,i] + 
                           a.v*pref.ben*A.u*expax*(V[,i]%*%gphi)*U[,i] + a.h*pref.herb*A.u*expax*(H[,i]%*%gphi)*U[,i])
      
      (a.u*pref.pel*A.u*expax)*(U[,i]*dx)%*%(gphi)
      
      
      # biomass density defecated by predators (g.m-3.yr-1)
      defbypred <- 10^x*(def.high*a.u*pref.pel*A.u*expax*(U[,i]%*%gphi)*U[,i] + 
                         def.low*a.v*pref.ben*A.u*expax*(V[,i]%*%gphi)*U[,i] + 
                         def.low*a.h*pref.herb*A.u*expax*(H[,i]%*%gphi)*U[,i])
      ...
      #### Detritus Biomass Density Pool ####
        # fluxes in and out (g.m-3.yr-1) of detritus pool and solve for detritus biomass density in next time step 
        # considering pelagic feces as input as well as dead bodies from both pelagic and benthic communities 
        # and phytodetritus (dying sinking phytoplankton)
      
      if (det.coupling==1.0) input.w <- sinking.rate*(sum(defbypred[ref:end]*dx)+ sum(OM.u[1:end]*U[1:end,i]*10^(x[1:end])*dx) 
                                                    + sum(SM.u[1:end]*U[1:end,i]*10^(x[1:end])*dx)
                                                    + sum(OM.h[1:end]*H[1:end,i]*10^(x[1:end])*dx) 
                                                    + sum(SM.h[1:end]*H[1:end,i]*10^(x[1:end])*dx))
                                                    + sum(OM.v[1:end]*V[1:end,i]*10^(x[1:end])*dx) 
                                                    + sum(SM.v[1:end]*V[1:end,i]*10^(x[1:end])*dx) 
      
      
      if (det.coupling==0.0) input.w <- sum(OM.v[1:end]*V[1:end,i]*10^(x[1:end])*dx) 
                                      + sum(SM.v[1:end]*V[1:end,i]*10^(x[1:end])*dx) 
      ...
      W[i+1] = W[i] + dW*dt        #biomass density of detritus g.m-3
      ...
      # (number per m-3)- solve for time + dt using implicit time Euler upwind finite difference (help from Ken Andersen and Richard Law)
      ...
      #### Herbivore Density ##########################################################################################
      # (nos.m-3) - same algorithm as above
    ...
    # To match data of fish that are fishable
    Pred_bio_fishable <- colSums(U[Fref:end,]*dx*10^x[Fref:end])
    Herb_bio_fishable <- colSums(H[Fref:end,]*dx*10^x[Fref:end])
    ...
    totaleatenbyben  <- sum((1/K.d)*10^x[(ref.det+1):end]*GG.v[(ref.det+1):end,N-1]*V[(ref.det+1):end,N-1]*dx)   
    # consumption rate g.m-3.yr-1
    ...
    return(list(Body.size = x, Preds=U[,N-1], P_grth=GG.u[, N-1], P_mrt=PM.u[, N-1],
                ...
                Pred_gm = Pred_bio[length(Pred_bio)], 
                Herb_gm = Herb_bio[length(Herb_bio)], 
                Inv_gm = Inv_bio[length(Inv_bio)],
                Pred_dat_gm_5 = Pred_dat_bio_5[length(Pred_dat_bio_5)], 
                Herb_dat_gm_5 = Herb_dat_bio_5[length(Herb_dat_bio_5)],
                Pred_gm_fishable = Pred_bio_fishable[length(Pred_bio_fishable)], 
                Herb_gm_fishable = Herb_bio_fishable[length(Herb_bio_fishable)], 
                Pred_prod = P_prod_gr[length(P_prod_gr)-1], 
                Herb_prod = H_prod_gr[length(H_prod_gr)-1], 
                Inv_prod = I_prod_gr[length(I_prod_gr)-1],
                Fpred_prod = FP_prod_gr[length(FP_prod_gr)-1], 
                Fherb_prod = FH_prod_gr[length(FH_prod_gr)-1],  
                Yield_p = Yp[,N-1], 
                Yield_h = Yh[,N-1], 
                W = W[N-1], A = A[N-1], 
                p_bio_time = Pred_bio, h_bio_time = Herb_bio, i_bio_time = Inv_bio, 
                ...
                params=params,
                FPred_prod_time=FP_prod_gr,
                FHerb_prod_time=FH_prod_gr,
                Inv_prod_time=I_prod_gr
    ))
    
    #Pred_gm = predator biomass (g per metre square)
    #Pred_dat_gm_5 = Predator biomass greater than 5cm (data only starts at 5cm)
    #Pred_prod = Predator productivity (total)
    #Fpred_prod = Predator fisheries productivity (productivity above size min.fishing.size)
    
    
    # if you want to save the whole time series:
    
    # return(list(Body.size = x, Preds=U[,1:N], P_grth=GG.u[, 1:N], P_mrt=PM.u[, 1:N],
    #Herbs=H[,1:N], H_grth=GG.h[,1:N], H_mrt=PM.h[,1:N],
    #Invs=V[,1:N], I_grth=GG.v[,1:N], I_mrt=PM.v[,1:N], 
    ...
    #Yield_p = Yp[,1:N], Yield_h = Yh[,1:N], W = W[1:N], A = A[1:N], p_bio_time = Pred_bio, h_bio_time = Herb_bio, i_bio_time = Inv_bio, 
    #A_time = Al_bio, Det_time = Det_bio,
    #Pred_consump = totaleatenbypred,
    #pred_by_size = eatenbysize,
    #Invert_consump = totaleatenbyben,
...
plotsizespectrum<-function(modeloutput,params,timeaveraged=F, add.points=F) {
  ...
  with(params, {  
    ...
    plot(log10(modeloutput$Preds)[ref:end]~modeloutput$Body.size[ref:end],
         xlab = "", ylab = "",
         type = "l", xlim = c(-3.5,4.5), ylim = c(-10,5), col = "red3")
    points(log10(modeloutput$Herbs)[ref.herb:end]~modeloutput$Body.size[ref.herb:end],
           type = "l", col = "green3")
    points(log10(modeloutput$Invs)[ref.det:end]~modeloutput$Body.size[ref.det:end], 
           type = "l", col = "blue3")
...
plotsizespectrum_full<-function(modeloutput,params,timeaveraged=F) {
  ...
  with(params, {  
    ...
    plot(log10(modeloutput$Preds)~modeloutput$Body.size, type = "l", 
         xlab = "", ylab = "", xlim = c(-12,4.5), ylim = c(-10,20), col = "red")
    points(log10(modeloutput$Herbs)[ref.herb:end]~modeloutput$Body.size[ref.herb:end], 
           type = "l", col = "green",xlab = "", ylab = "")
    points(log10(modeloutput$Invs)[ref.det:end]~modeloutput$Body.size[ref.det:end], 
           type = "l", col = "blue")
...
plotbiomass <- function(modeloutput, params){
  ...
  with(params, {
    ...
    plot(modeloutput$p_bio_time, type = "l", col = "red3", xlab = "", ylab = "", 
         ylim = c(0,max(modeloutput$p_bio_time+10)))
    points(modeloutput$h_bio_time, type = "l", col = "green3")
    points(modeloutput$i_bio_time, type = "l", col = "blue2")
